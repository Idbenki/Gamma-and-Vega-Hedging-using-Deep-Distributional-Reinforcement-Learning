...
def _ensure_torch_utils_loaded():
    # Some kernel states can lose lazy torch internals after package upgrades.
    # Importing torch._utils explicitly prevents optimizer init from crashing.
...
def compute_state_scale(params):
    ...
    # Each client contract contributes contract_size units, so portfolio greeks are
    # contract_size times larger than per-unit greeks.
    contract_size = float(params.get("contract_size", 1.0))
    port_scale = max(np.sqrt(lam * T_days), 1.0) * contract_size

    gamma_only_state = bool(params.get("gamma_only_state", False))
    if gamma_only_state:
        return np.array(
            [
                S0,                    # S feature
...
def train(
    ...
    for ep in range(int(n_episodes)):
        ...
        if int(log_every) > 0 and (ep + 1) % int(log_every) == 0:
            print(
